...
<span id="24"><h4>24. 闵可夫斯基</h4></span>
求b+w=a？
w=a-b
C={a+(-b)}构造的闵可夫斯基和
判断w在C中不
...
struct Point{
    ...
    bool operator==(const Point p)const{return cmp(x,p.x)==0&&cmp(y,p.y)==0;}
    bool operator<(const Point p)const{if(cmp(x,p.x)==0) return y<p.y;return x<p.x;}
}p1[MAXN],p2[MAXN],p,p3[MAXN<<1],stk[MAXN<<1];
bool cmp2(Point A,Point B) {return (A^B)>0||((A^B)==0&&A.len()<B.len());}
int Andrew(Point *p,Point *stk,int n) {
    sort(p + 1, p + 1 + n);
    int len = 0;
    for (int i = 1; i <= n; i++) {
        while (len > 1 && sgn((stk[len] - stk[len - 1]) ^ (p[i] - stk[len - 1])) <= 0) len--;//看这
        stk[++len] = p[i];
    }
    int k = len;
    for (int i = n - 1; i >= 1; i--) {
...
int Minkowski(Point *C1,Point *C2,Point *A,int n,int m){
...
int in(Point a,Point *A,int tot){
    if((a^A[1])>0||(A[tot]^a)>0) return 0;
    int ps=lower_bound(A+1,A+tot+1,a,cmp2)-A-1;
    return ((a-A[ps])^(A[ps%tot+1]-A[ps]))<=0;
}
void solve(){
    n=Andrew(p1,stk,n);
    m=Andrew(p2,stk,m);
    int len=Minkowski(p1,p2,p3,n,m);
    len=Andrew(p3,stk,len);
    Point k=p3[1];
    rep(i,1,len){
        p3[i]=p3[i]-k;
    }
